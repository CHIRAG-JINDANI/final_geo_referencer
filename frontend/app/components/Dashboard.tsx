...
export default function Dashboard() {
  ...
  const handleProcess = useCallback(async () => {
    if (!referenceFile || !capturedProxy) return
    setStage('processing')
    setResult(null)
    addLog('info', 'dispatching to python pipeline...')
    addLog('dim', '  [1/5] preprocessing: CLAHE + bilateral filter')
    ...
    try {
      ...
      // Simulate log steps while waiting
      const logSteps = [
        [800, '  [2/5] SIFT keypoint detection across 4×4 grid'],
        [1600, '  [3/5] brute-force matching + Lowe ratio test (0.75)'],
        [2400, '  [4/5] RANSAC homography — computing warp matrix'],
        [3200, '  [5/5] warping + blending → building GeoTIFF'],
      ]
      logSteps.forEach(([delay, msg]) => {
        setTimeout(() => addLog('dim', msg as string), delay as number)
      })
      ...
      setResult(data)
      setStage('preview')
      addLog('ok', `pipeline complete — ${data.inlierCount} inlier matches`)
      addLog('ok', `match score: ${(data.matchScore * 100).toFixed(1)}%`)
  ...
  }, [referenceFile, capturedProxy, viewport, addLog])
